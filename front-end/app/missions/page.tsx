...
const groupedMissions: Record<string, Mission[]> = {
  ...
  "On-Chain Missions": missions.filter(
    (_: Mission, index: number) => index >= 5
  ),
};
...
export default function MissionsPage() {
  ...
  ); // Set to the specific wallet address
  ...
  useEffect(() => {
    ...
    loadMissionProgress();
  }, [claimedMissions, userAddress, user, loadMissionProgress]);
  ...
  const handleClaimReward = (mission: Mission) => {
    ...
    if (mission.onchainVerification) {
      ...
      if (!progress || !progress.isCompleted) {
        showToast(
          "Mission incomplete",
          "Complete the on-chain requirements first"
        );
        return;
      }
    }
    ...
    showToast(
      "Reward claimed!",
      `You've successfully claimed ${mission.reward}`
    );
  };
  ...
  const getMissionsToDisplay = (): Mission[] => {
    ...
    return (
      groupedMissions[activeCategory as keyof typeof groupedMissions] || []
    );
  };
      ...
      >
            ...
            <div className="flex items-center bg-blue-100 px-3 py-1 rounded-lg">
              ...
              <span className="text-sm font-medium text-blue-800">
                {isLoading ? (
                  ...
                  <span>
                    {Object.values(missionProgress).find(
                      (m) => m?.progress?.current > 0
                    )?.progress.current || 0}{" "}
                    Mints
                  </span>
                )}
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-medium">
                {claimedMissions.length} / {missions.length}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {missionsToDisplay.map((mission: Mission) => {
            ...
            const onchainProgress = mission.onchainVerification
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2 mt-3">
                      <div
                        className={`h-2 rounded-full transition-all duration-500 ${
                          isClaimed ? "bg-green-600" : "bg-blue-600"
                        }`}
                        style={{
                            ...
                            <div className="flex flex-col space-y-2">
                              <div className="flex justify-between items-center">
                                <span className="text-sm text-gray-600">
                                  {mission.onchainVerification.action ===
                                  "claim"
                                    ? "Token Claims"
                                    : "Actions"}
                                  :
                                ...
                                <span className="font-medium text-sm">
                                  {isLoading
                                    ? "Loading..."
                                    : `${
                                        onchainProgress?.progress.current || 0
                                      } / ${
                                        mission.onchainVerification
                                          .requiredCount
                                      }`}
